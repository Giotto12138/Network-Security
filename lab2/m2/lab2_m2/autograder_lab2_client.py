...
import autograder_lab2_packets as autograder_packets
...
class Lab2TestProtocol_Server(asyncio.Protocol):
    ID = 0
    def __init__(self):
...
class Lab2AutogradeClient(asyncio.Protocol):
        ...
        for packet in self.deserializer.nextPackets():
            if isinstance(packet, autograder_packets.AutogradeTestStatus):
                if packet.FAILED in [packet.submit_status, packet.client_status, packet.server_status]:
                    print("Lab2 Autograde test failed because server says", packet.error)
                    self.transport.close()
                    return
                elif self.state == "test_start" and packet.submit_status == packet.NOT_STARTED:
                    print("Lab2 submission process didn't start for some reason.")
                    self.transport.close()
                    return
                elif self.state == "client_test" and packet.client_status == packet.NOT_STARTED:
                    print("Lab2 client test process didn't start for some reason.")
                    self.transport.close()
                    return
                elif self.state == "server_test" and packet.server_status == packet.NOT_STARTED:
                    print("Lab2 server test process didn't start for some reason.")
                    self.transport.close()
                    return
    ...
    async def do_client_test(self, client_command):
        ...
        try:
            transport, protocol = await playground.create_connection(
                DummyProtocol,
                host="crap_shan_m2://{}".format(self.server_addr), 
    ...
    async def do_server_test(self, server_command):
        if self.server == None:
            print("No server. Start Cra[ server")
            try:
