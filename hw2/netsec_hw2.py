...
"""
import random, sys
import socket           #引入socket包
import time     #引入time包，使用sleep函数
...
class EscapeRoomObject:
    ...
    def do_trigger(self, *trigger_args):
        return [event for trigger in self.triggers for event in [trigger(self, *trigger_args)] 
if event]
    ...
    def _cmd_get(self, get_args):
        if len(get_args) == 0:
            ...
            get_result = (
                ...
                ((not object or not object["visible"])             and "You don't see that") or                ((not object["gettable"])                          and "You can't get that.") or
                                                                   success_result)
...
def main(args):
    ...
    s=socket.socket()   #创建并连接socket，设定ip地址和端口号（由服务器那端提供
    s.connect(("192.168.200.52",19002))

    print(s.recv(1024))#接收数据并打印查看
    time.sleep(0.5)#client睡眠等待接收到server那边的信息
    s.send(b"Qingshan Zhang<EOL>\n")#一次一次发送信息通过游戏
                        #设定使用EOL作为句尾的标记，方便客户端收到信息后分割
    print(s.recv(1024))#1024是可接收的最大byte数，1024足够
    time.sleep(0.5)
    s.send(b"look<EOL>\n") #使用b，把字符串转换成byte类型
    ...
    def respond(msg): #自定义一个函数
        msg=msg+'<EOL>\n'
        print('msg:',msg)
        s.send(msg.encode()) #发送服务端的返回值


    game = EscapeRoomGame(output=respond) #建立一个ERG object，并且自定义一个函数来自定义操作  
    game.create_game(cheat=("--cheat" in args))
    game.start()

    while game.status == "playing": #建立一个循环，服务端监听客户端是否还在游戏中

        info=s.recv(1024).decode() #接收客户端的输入，同时把byte转成string类型
        time.sleep(0.5)        #等待客户端的输入
        lines=info.split('<EOL>\n') #通过EOL分割输入，分成一句一句
        lines=list(filter(None,lines))#判断lines里是否有空语句，过滤掉空语句
        print("lines: ",lines)

        for line in lines:      #在lines里遍历line，一行一行读语句
            if line:
                print("command:",line)
                game.command(line)#程序根据语句得出对应的输出
...
if __name__=="__main__":
    main(sys.argv[1:])  #主函数入口，接收从外部获取的一个列表作为参数，【1：】代表从第一个参数开始接收